...
| T004 | verified | Claude Code | T003 | src/components/map/MapSection.tsx, tasks.md, docs/decisions.md | Map polls `/api/emotions` every 60s (skipped while tab hidden) so colored emotion data updates live without a page reload, in addition to already-existing color-coded rendering | pnpm lint; pnpm build | 2026-08-06: pnpm lint passed; pnpm build passed with existing edge-runtime static-generation warning. Verified prod `www.worldemomap.com/api/emotions` currently returns `{"data":[],"message":"Supabase connection is temporarily paused"}` — polling is live in code but blocked from showing real data until a human unpauses Supabase in prod (`NEXT_PUBLIC_SUPABASE_PAUSED`). |
