...
**User Story**: ログインユーザーとして、関心のある国・セクター・組合せを保存して素早くアクセスしたい
**Acceptance Criteria**:
- Given: ログイン済みで国詳細パネルを見ている時
- When: ☆ボタンをクリックする
- Then: その国がお気に入りに追加される。☆が★に変わる
- Given: セクター詳細パネルでお気に入りを追加する時
- When: 大分類または小分類の☆をクリックする
- Then: そのセクターがお気に入りに追加される
- Given: /favorites ページにアクセスした時
- When: お気に入り一覧が表示される
- Then: 保存した国・セクター・組合せがカード形式で表示され、各カードに現在の感情サマリーが表示される
